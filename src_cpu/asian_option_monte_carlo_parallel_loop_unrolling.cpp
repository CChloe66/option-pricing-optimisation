...
// Computes the Asian option price using Monte Carlo simulation with loop unrolling
double asian_option_monte_carlo(int num_paths, double S0, double K, double T, double r, double sigma, int num_points) {
    ...
    #pragma omp parallel reduction(+:total_payoff)
    {
        ...
        for (int i = 0; i < num_paths; i++) {
            double path_sum = 0.0;
            double S = S0, S_next;
            for (int j = 0; j < num_points; j += 2) { // Loop with step 2 for unrolling
                double z = gaussian_rng(&seed);
                S *= exp(drift + vol * z);
                path_sum += S;
                if (j + 1 < num_points) {  // Ensure we do not go out of bounds
                    z = gaussian_rng(&seed);
                    S *= exp(drift + vol * z);
                    path_sum += S;
                }
